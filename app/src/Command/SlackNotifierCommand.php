...
use App\Service\PrestaShop\ModuleFetcher;
use App\Service\PrestaShop\ModuleFlagAndRate;
use App\Service\PrestaShop\NightlyBoard;
...
class SlackNotifierCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->moduleChecker = new ModuleFlagAndRate();
        $this->moduleFetcher = new ModuleFetcher();
    ...
    protected function checkStatusNightly(): string
    {
        ...
        foreach (self::BRANCH_SUPPORT as $branch) {
            foreach (self::CAMPAIGN_SUPPORT as $campaign) {
                ...
                $status = ($hasFailed && 0 == $report['tests']['failed']);
    ...
    protected function checkPRNaming(): array
    {
        ...
        foreach ($arrayPullRequest as $pullRequest) {
            ...
            foreach (array_keys($arrayTeamPR) as $maintainer) {
                ...
                if (self::NUM_PR_FOR_MAINTAINERS == count($arrayTeamPR[$maintainer])) {
                    continue;
                }
    ...
    {
        // Next version
        $content = $this->github->getRepoEndpoint()->contents()->download('PrestaShop', 'PrestaShop', 'app/AppKernel.php', 'refs/heads/develop');
        ...
        foreach ($results as $key => &$result) {
            if ('prestashop-specs' == $result['node']['repository']['name']) {
                unset($results[$key]);
                continue;
            }
            // Issue
            ...
            $result['priority'] = array_reduce($issueLabels, function ($carry, $item) {
                if ($carry < 2 && 'Must-have' === $item) {
                    return 2;
                }
                if ($carry < 1 && 'Nice-to-have' === $item) {
                    return 1;
                }
            ...
            // Milestone
            $result['milestone'] = '';
            if ('PrestaShop' == $result['node']['repository']['name']) {
        ...
        usort($results, function ($a, $b) {
            ...
            // #1 : Core, Modules
            if ('PrestaShop' === $aRepoName && 'PrestaShop' !== $bRepoName) {
                return -1;
            }
            if ('PrestaShop' === $bRepoName && 'PrestaShop' !== $aRepoName) {
                return 1;
            }
            if ('PrestaShop' === $aRepoName) {
                // #2 : Milestone
        ...
        foreach ($results as $key => $pullRequest) {
            ...
            if ($milestone !== $pullRequest['milestone'] || 3 == $milestoneCount) {
            ...
            $slackMessage .= ' - '
                ...
                .($pullRequest['priority'] > 0 ?
                    ('*_['.(2 === $pullRequest['priority'] ? 'Must Have' : 'Nice-to-have').']_* ')
                    : ''
                )
