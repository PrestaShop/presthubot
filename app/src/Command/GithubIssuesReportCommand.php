...
class GithubIssuesReportCommand extends Command
{
    ...
    {
        if (null === $input->getOption('dateStart')) {
        ...
        if (null !== $input->getOption('dateEnd')) {
    ...
    private function generateReport(InputInterface $input, OutputInterface $output): void
    {
        ...
        foreach ($issues as $issue) {
            ...
            foreach ($issue['labels']['nodes'] as $label) {
                if ('Regression' === $label['name']) {
                    $this->results['regressions'][] = $issue;
                }
                if ('Detected by TE' === $label['name']) {
                    ++$this->results['regressions_TE'];
                }
                if ('Duplicate' === $label['name']) {
        ...
        foreach ($this->results['duplicates'] as $duplicate) {
            $comments = $this->github->getIssueEndpoint($this->github->getClient())->comments()->all('PrestaShop', 'PrestaShop', $duplicate['number']);
            foreach ($comments as $comment) {
                preg_match('/Duplicates? of #(\d+)/i', $comment['body'], $matches);
                if (isset($matches[1])) {
                    // We got the original issue number
                    $issueNumOriginal = $matches[1];
                    $issueOriginal = $this->github->getIssueEndpoint($this->github->getClient())->show('PrestaShop', 'PrestaShop', $issueNumOriginal);
