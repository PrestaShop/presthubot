...
class GithubStatsCommand extends Command
{
    ...
    private function getStats(InputInterface $input, OutputInterface $output)
    {
        ...
        $repository = $this->github->getRepoEndpoint()->show('PrestaShop', 'PrestaShop');

        $rows[] = ['# Stars', $repository['stargazers_count']];
        $rows[] = ['# Issues', $repository['open_issues_count']];

        $openPullRequests = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:pr is:open');
        ...
        if ($input->getOption('username')) {
            ...
            $openPullRequests = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:pr is:open author:'.$input->getOption('username'));
            $rows[] = ['# PR Open', $openPullRequests['total_count']];
            $mergedPullRequests = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:pr is:merged author:'.$input->getOption('username'));
            $rows[] = ['# PR Merged', $mergedPullRequests['total_count']];
            $closedPullRequests = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:pr is:closed is:unmerged author:'.$input->getOption('username'));
            $rows[] = ['# PR Closed', $closedPullRequests['total_count']];
            $rows[] = new TableSeparator();

            $openIssues = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:issue is:open author:'.$input->getOption('username'));
            $rows[] = ['# Issue Author Open', $openIssues['total_count']];
            $closedIssues = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:issue is:closed author:'.$input->getOption('username'));
            $rows[] = ['# Issue Author Closed', $closedIssues['total_count']];
            $rows[] = new TableSeparator();

            $openIssues = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:issue is:open assignee:'.$input->getOption('username'));
            $rows[] = ['# Issue Assignee Open', $openIssues['total_count']];
            $closedIssues = $this->github->getSearchEndpoint()->issues('repo:PrestaShop/PrestaShop is:issue is:closed assignee:'.$input->getOption('username'));
