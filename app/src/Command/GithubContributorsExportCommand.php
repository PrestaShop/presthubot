...
class GithubContributorsExportCommand extends Command
{
    ...
    private function export(InputInterface $input, OutputInterface $output): void
    {
        ...
        foreach ($contributors as $author => $agency) {
            $pullRequests = $this->github->getSearchEndpoint($this->github->getClient())->issues('org:PrestaShop is:pr author:'.$author);
            ...
            foreach ($pullRequests['items'] as $pullRequest) {
                ...
                $apiPullRequest = $this->github->getPullRequestEndpoint($this->github->getClient())->show('PrestaShop', $repository, $pullRequest['number']);
                ...
                if ($linkedIssue) {
                    $severityIssue = array_reduce($linkedIssue['labels'], function (string $carry, array $item) {
                        ...
                        if (0 === strpos($item['description'], 'Severity')) {
                            return $item['name'];
                        }
